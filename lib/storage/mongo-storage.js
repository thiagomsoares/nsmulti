    ...
    if (mongo.db != null && !forceNewConnection) {
      ...
      const connect_with_retry = async function (i) {
        ...
        try {
          ...
          const dbName = mongo.client.s.options.dbName || 'nightscout';
          ...
          // Teste de escrita para verificar permissões
          try {
            await mongo.db.collection('connection_test').insertOne({ test: true, date: new Date() });
            console.log('MongoDB write test successful');
          } catch (writeErr) {
            console.warn('MongoDB write test failed, but continuing: ', writeErr.message);
          }
